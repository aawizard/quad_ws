...
class PID_alttitude:
    ...
    def step(self, error, curr):
        ...
        total_output = (pid_output + self.feedforward) * 0.6
         ...
         # Smoothing output
        smoothed_output = 0.9 * total_output + 0.1 * self.prev_output

        # Clamp and scale output
        # smoothed_output = max(min(smoothed_output, self.max_thrust), 0)
        # output = int((smoothed_output / self.max_thrust) * 1000) + 1000
